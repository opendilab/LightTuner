...
# The following implementation of custom MPQueue to avoid NotImplementedError
...
class MPQueue(Queue):
    ...
    serialize MPQueue when it is passed between processes. If these functions
    are not defined, MPQueue cannot be serialized, which will lead to the error
    of "AttributeError: 'MPQueue' object has no attribute 'size'".
    ...
    def __getstate__(self):
        """Help to make MPQueue instance serializable.
        Note that we record the parent class state, which is the state of the
        actual queue, and the size of the queue, which is the state of MPQueue.
