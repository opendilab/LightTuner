...
class _MySession(BaseSession):
    ...
    def _run(self):
        for i in range(self.__algorithm.v):
            ...
            self._put_via_space(
                tuple(i for _ in range(len(self.vsp))), ('this is attachment', i)
            )
...
class TestHpoAlgorithmBaseAlgorithm:
    ...
    def test_base_algorithm_get_session(self):
        ...
        assert sorted(_session1.reslist) == [
            (Task(1, {'a': 0, 'b': (0, 0)}, ('this is attachment', 0)), 1),
            (Task(2, {'a': 1, 'b': (1, 1)}, ('this is attachment', 1)), 2),
            (Task(3, {'a': 2, 'b': (2, 2)}, ('this is attachment', 2)), 16),
            (Task(4, {'a': 3, 'b': (3, 3)}, ('this is attachment', 3)), 216),
            (Task(5, {'a': 4, 'b': (4, 4)}, ('this is attachment', 4)), 4096),
            (Task(6, {'a': 5, 'b': (5, 5)}, ('this is attachment', 5)), 100000),
            (Task(7, {'a': 6, 'b': (6, 6)}, ('this is attachment', 6)), 2985984),
            (Task(8, {'a': 7, 'b': (7, 7)}, ('this is attachment', 7)), 105413504),
            (Task(9, {'a': 8, 'b': (8, 8)}, ('this is attachment', 8)), 4294967296),
            (Task(10, {'a': 9, 'b': (9, 9)}, ('this is attachment', 9)), 198359290368),
        ]
    ...
    def test_base_algorithm_get_session_with_error(self):
        ...
        assert sorted(_session1.errlist) == [
            (Task(3, {'a': 2, 'b': (2, 2)}, ('this is attachment', 2)), OverflowError, ('this is overflow', 2)),
            (Task(6, {'a': 5, 'b': (5, 5)}, ('this is attachment', 5)), OverflowError, ('this is overflow', 5)),
            (Task(9, {'a': 8, 'b': (8, 8)}, ('this is attachment', 8)), OverflowError, ('this is overflow', 8)),
        ]
    ...
    def test_base_algorithm_get_session_with_session_error(self):
        ...
        _service1 = _TestThreadService(_my_func)
        _session1 = _MyAlgorithm(10, even_only=True).get_session({
            'a': uniform(0, 10),  # these uniform spaces are only placeholders here
            'b': (
                uniform(0, 10),
                uniform(0, 10),
